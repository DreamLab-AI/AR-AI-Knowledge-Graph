...
const fields: RegistryField[] = [
  ...
  { key: 'maxNodeCount', subgroup: 'GPU & Quality Gates', label: 'Max Node Count', type: 'slider', min: 1000, max: 500000, step: 5000, path: 'qualityGates.maxNodeCount', description: 'Maximum nodes to render (set high to show all)' },
  ...
  { key: 'groupingMethod', subgroup: 'Run Grouping', label: 'Method', type: 'select', localKey: 'method', options: ['communities', 'kmeans', 'dbscan'], description: 'How to group nodes: "communities" = topological community detection (Leiden) coloured by partition; "kmeans" = spatial K-means into a fixed number of clusters; "dbscan" = density-based spatial clustering (auto cluster count, marks outliers).' },
  { key: 'groupingNumClusters', subgroup: 'Run Grouping', label: 'Cluster Count (K)', type: 'slider', localKey: 'numClusters', min: 2, max: 50, step: 1, showWhen: { localKey: 'method', equals: 'kmeans' }, description: 'Number of spatial clusters for K-means (K).' },
  { key: 'groupingEps', subgroup: 'Run Grouping', label: 'Neighbourhood (eps)', type: 'slider', localKey: 'eps', min: 0.1, max: 10, step: 0.1, showWhen: { localKey: 'method', equals: 'dbscan' }, description: 'DBSCAN neighbourhood radius — larger merges more nodes into each cluster (backend range 0.1–10).' },
  { key: 'groupingMinSamples', subgroup: 'Run Grouping', label: 'Min Points', type: 'slider', localKey: 'minSamples', min: 1, max: 50, step: 1, showWhen: { localKey: 'method', equals: 'dbscan' }, description: 'DBSCAN minimum points to form a dense region — higher leaves more nodes as outliers.' },
  { key: 'groupingResolution', subgroup: 'Run Grouping', label: 'Resolution', type: 'slider', localKey: 'resolution', min: 0.1, max: 5, step: 0.1, showWhen: { localKey: 'method', equals: 'communities' }, description: 'Community-detection resolution — higher yields more, smaller communities (backend range 0.1–10). Independent of the Physics-tab cohesion resolution.' },
