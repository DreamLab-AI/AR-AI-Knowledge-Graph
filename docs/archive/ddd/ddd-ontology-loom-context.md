...
- **I11 — The Loom is the ONTOLOGY only; it never touches the working graph.** The Loom's read-truth store and every façade surface (`/loom/sparql`, `/loom/search`, `/loom/scaffold`, distillation) serve the **published ontology generation only** (`ontology.ttl` + `ontology-inferred.ttl` + indexes). The Loom **never reads, mirrors, ingests, or queries `workingGraph`** — the personal/working knowledge graph, which may become **multi-user or private** graphs. A Generation is public-ontology by construction (the pipeline builds it from `mainKnowledgeGraph`, not `workingGraph`), and `mirror.sh` pulls only those artifacts; this invariant forbids ever widening that. **Corollary — the three uplift doors:** knowledge reaches the ontology *only* through (1) VisionClaw's governed propose door, (2) the forum/ACSP governance surface, or (3) direct agentic action in the agentbox container writing into the `logseq` corpus dir and pushing. All three land in `mainKnowledgeGraph`, are reasoned in CI, and become a new Generation the Loom then **mirrors** — read-only, downstream, always. Examining the working graph is **VisionClaw's** responsibility (BC-outside-Loom), never the Loom's. This is a structural privacy/multi-tenancy boundary, not a policy that can erode.
