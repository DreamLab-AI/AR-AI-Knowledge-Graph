...
>
> **First in-headset render — VIVE Pro bring-up (2026-08-22, ADR-136, branch `xr-vive-runtime`).** The Godot XR client rendered both eyes on a physical HTC VIVE Pro for the first time: **Godot 4.6.1 + the Compatibility (OpenGL) renderer + NVIDIA 580 + native X11 + SteamVR desktop OpenXR**. Reality corrections to the runtime/renderer/interaction context below:
> - **Renderer.** The desktop working path is the **Compatibility renderer, not Forward+/RD Vulkan.** The RD/Vulkan **multiview tonemapper is broken on this stack** (submission fails under Compat-XR multiview), so glow/bloom is **off** and Compatibility is mandatory for the desktop VIVE path. The §8-era assumption of the Quest `mobile` (Forward Mobile) renderer holds only for the frozen APK; on-desktop it is Compatibility. So `OpenXRRuntime` here resolves to **`SteamVR`**, not Monado/Meta, and the `runtime = SteamVR`, `performance_tier = Tier3Tethered` branch is now the exercised one.
> - **Interaction.** Dual-wand interaction is live (right wand confirmed); `openxr_action_map.tres` is bound to `htc/vive_controller` with `pose="aim"`. Grab is a **literal world-space ray/sphere intersection** (best-aligned pick; `interaction.rs` `TARGET_RADIUS` tightened 1.0→0.05) that **keeps the grab distance along the ray**; **trackpad locomotion** and a **world-anchored, wand-grabbable (movable) HUD** are in. Left wand + edge-tracking + adaptive-fit-on-grab are WIP.
> - **Client render model.** The client now runs **adaptive room-fit** (scales/recentres the streamed graph into a room-sized volume, re-measured each frame so it tracks physics spread) and **client-side optimistic position hunting** (`_node_targets → eased _node_positions`) — the desktop "**GPU settles fast, clients hunt at their own speed**" model. Backend now defaults to **Continuous settle** (was a FastSettle latch that froze the graph), `PinNodePositions` injects a dragged node's position into the GPU so it perturbs neighbours' springs (grab GPU-injection), and `initialGraphLoad` node limit was raised 200→3000 so edges reach the client (top-640 / top-5% node display, edges only between displayed nodes). Canaries pending; the **Quest 3 APK remains frozen and is still the sole ship target** (ADR-136 §2).
