...
> **Related**: [PRD](../prd/prd-bead-provenance-upgrade.md) · [ADR-034](../adr/ADR-034-needle-bead-provenance.md) · [Neo4j Schema §2d](../reference/graph-schema.md#8-provenance-graph) · [NEEDLE](https://github.com/jedarden/NEEDLE)
...
|------|-------|------|-------|
| `bead_types.rs` | 427 | Domain types — FSM, outcomes, metadata, learning, retry config | 14 |
| `bead_store.rs` | 1,217 | `BeadStore` trait + `Neo4jBeadStore` + `NoopBeadStore` + `MockBeadStore` | 19 |
| `bead_lifecycle.rs` | 460 | `BeadLifecycleOrchestrator` — state machine coordination | 7 |
