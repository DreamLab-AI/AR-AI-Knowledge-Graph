...
**Acceptance condition:** Record the exported client, gdext library, server protocol, driver/runtime and headset revisions; verify stereo submission, frame budget, reconnect and input on that combination. Test mobile separately before changing its frozen declaration. Reopen on renderer/runtime, export, shader or target changes. See [XR coverage review](https://github.com/DreamLab-AI/VisionFlow/blob/main/docs/estate-review/rendered-state.md#xr-control-coverage-and-hierarchy-semantics) and [receipt](https://github.com/DreamLab-AI/VisionFlow/blob/main/docs/estate-review/evidence/xr-decision-probe.json). No Godot, Android or headset execution ran.
