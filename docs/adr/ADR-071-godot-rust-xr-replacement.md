...
**Implementation status (2026-07-08, gap-close `gap-close/2026-07-xr`):** A **copresence layer** landed *after* Phase 2 under ADR-130 (this ADR's gap-close child decisions), driven by `scratchpad/xr-copresence-research-brief.md`. ADR-130 Decision 1 keeps MR on this Godot surface and neutralises the WebXR VR entry in place (deletion still deferred to Phase 3); Decision 4 fixes the copresence design (minimal geometric agent avatars, head-gaze-primary selection with eye-gaze as a Quest-3-gated progressive enhancement, Hall's-zones proxemics on a forward arc, three selection resolvers into one arbiter). The Rust sim/GDScript rig split from Option 1 holds: new `xr-client/rust/src/{gaze,selection,proxemics,avatar_state}.rs` and a 0x44 agent-presence wire (sibling of the 0x43 pose frame) own the state machines, gaze resolution, proxemics and selection arbitration; the `.tscn`/GDScript layer owns the rig, action map and render. This closes register items M1/M2/COM-18/M4/M6 at `integrated` against the Godot client and M3 at the geometric subset (`standalone` — see below), all **code + test-proven headless** (`cargo test` green; host `.so` release artefact built). **On-device validation is pending:** no `godot` binary runs in this container, so every in-headset render and each live canary fire (`CANARY-VC-M1-HUD`, `CANARY-VC-COM18-INTERV`, `CANARY-VC-M4-RAY`) is validated only in the **agentbox/xr-runtime Monado sidecar** (Godot 4.3, VNC :5904); the `aarch64-linux-android` target and `cargo-ndk` are not installed here (recorded, not worked around), so the APK cross-build is likewise unproven and the release proof is a host-target `.so`. Per-item tiers + evidence: `docs/gap-close-evidence/P2-{M1,M2,M3,M4,M6}.md`. This ADR's core decision (Godot + gdext + OpenXR, Rust-for-substrate / GDScript-for-rig) is unchanged; the copresence design is recorded in child ADR-130 Decision 4, with one transport deviation noted in its ADR-trail (the M2 decide POST uses Godot `HTTPRequest` + a Rust-signed NIP-98 header rather than a new `reqwest` client, keeping the secret key off the GDExtension boundary).

**Implementation status (2026-08-22, branch `xr-vive-runtime`):** **First-ever working headset render of this client** — both eyes, live wand interaction, live physics — on an **HTC VIVE Pro via desktop OpenXR / SteamVR** (the ADR-136 validation target, *not* a Quest 3). This is the first time any in-headset render of the Godot client has run on real hardware rather than the Monado simulated-HMD sidecar. **A hard render constraint surfaced and shapes the renderer choice going forward:** the RenderingDevice renderers (Forward+/Mobile, Vulkan) have a **broken multiview tonemapper** on this SteamVR/Linux/NVIDIA stack — null shader variant → both eyes black, reproducible across Godot 4.3/4.4/4.6.1, NVIDIA 610 and 580, X11 and Wayland, all settings. The **Compatibility (OpenGL / `gl_compatibility`) renderer** (inline tonemap, no RD post pass) is the only one that renders correctly; glow / advanced post-process breaks its XR multiview submission (glow OFF, Linear tonemap only), and its second-eye `GL_OVR_multiview` needs NVIDIA **580** (610 fails it). Working stack: **Godot 4.6.1-stable** (note: this bring-up ran on 4.6.1, ahead of the 4.3.x pin in the Neutral consequences below — the pin should be revisited) + Compat renderer + NVIDIA 580.178.04 + native X11 (XFCE) + SteamVR on HP-Desktop. Validated in-headset: dual-wand path (right wand live, left pending power-on), ray/sphere node grab with keep-distance, trackpad locomotion, world-anchored wand-movable HUD, `htc/vive_controller` action map, top-5% node display (640 by centrality), adaptive room-fit, edges (initial-load node cap 200 → 3000). Backend physics changed to a **Continuous settle** default (replacing the FastSettle latch that froze the graph) with **grab-reheat** and a new **`PinNodePositions` GPU injection**. **What is NOT done — no over-claim:** the formal copresence canary session (`CANARY-VC-M1-HUD` / `-M4-RAY` / `-COM18-INTERV`) is **PENDING**, so no M-item promotes to `integrated` on the back of this session; the **Quest 3 APK stays frozen/unbuilt** (no `aarch64` NDK / `cargo-ndk`). WIP/known issues: edge endpoints don't track moved nodes, adaptive-fit rescales oddly during a grab, left wand unpowered, glow parity sacrificed on the Compat path, debug prints retained. Detail: PRD-008 §7.4; ADR-136 Outcome. Committed to `xr-vive-runtime` (pushed to `DreamLab-AI/VisionClaw`), PR-ready for `main`, still WIP.

**Implementation status (2026-08-30, branch `xr-vive-hardening`):** the VIVE render path was hardened from a capped-subset demo to a full-density client — recorded as its own decision in **ADR-137** (render offload, runtime quality dials, full-3D-default layout). Headlines: the per-frame position-hunt and MultiMesh packing moved from GDScript into a pure Rust `RenderStore` (`xr-client/rust/src/render_store.rs`, wrapped by `BinaryProtocolClient`), taking full density (**13,164 nodes / 145,692 edges**) from ~11 fps to **90 fps**; instance budgets are now **topology-derived** (bounded by a safety ceiling) rather than the hardcoded 640/3000 gates; the initial-load cap became the `initialNodeLimit` settings dial (`PUT /api/settings/physics`) with the WS receive limit raised to **256 MiB**; `axisCompressionZ` was removed and dual-disc flatten made opt-in (`enableDualDiscLayout` default OFF) so **natural 3D is the default layout**; the client decodes the **Protocol V5** position wrapper (`[0x05][u64 broadcast_seq][V3 records]`, ADR-102 amended); and XR-safe eye candy (fresnel-halo `next_pass`, TIME-driven edge-flow shader, centrality size tells) was added under the Compat no-post-process constraint. The HUD gained a working **wand-ray pointer** (first working VR button input), an operator control panel (Reset Layout / Spread± / Edges± / NodeSize±), proximity-gated node labels (pooled `Label3D`, 4 Hz), and a **double-click → narrativegoldmine page card** document view. The core decision (Godot + gdext + OpenXR) is unchanged. Detail: ADR-137; PRD-008 §7.4/§7.5.
...
- Godot version is pinned to 4.3.x for the lifetime of Phase 0–3. Major-version upgrade decisions go through a follow-up ADR. **Update (2026-08-22):** the first working headset bring-up ran on **Godot 4.6.1-stable**, ahead of this pin; the pin should be revisited given 4.6.1 is what renders on the VIVE hardware.
- **Renderer constraint on the SteamVR/Linux/NVIDIA validation stack (2026-08-22).** The Decision Drivers assumed Godot's Forward+ clustered (RenderingDevice) pipeline. On the VIVE Pro / SteamVR / NVIDIA desktop stack the RenderingDevice renderers have a **broken multiview tonemapper** (null shader variant → both eyes black); only the **Compatibility (OpenGL) renderer** renders correctly, at the cost of glow / advanced post-process (which breaks Compat XR multiview submission). This is a Linux/SteamVR/NVIDIA desktop constraint, not necessarily a Quest constraint — but it means the desktop-OpenXR validation vehicle runs a different renderer and reduced post-processing from the intended Quest target. Any shader/material porting (glass edges, instanced labels) must account for both renderer paths. Detail: ADR-136 Outcome, PRD-008 §7.4.
