...
|---|---|---|---|
| 1 | **ADR-111** | **DONE (2026-08-25).** Flip status line `Proposed → Accepted (implemented 2026-07-22)`; optionally fold in the receipt's deferral caveats. No code paths depend on it. Verified: status line already reads "Accepted — the §7 follow-up task ran 2026-07-22 ... status flipped 2026-08-21." | `docs/adr/ADR-111-ecosystem-infographic-modernisation.md:3` |
| 2 | **ADR-026** | **DONE (2026-08-25).** Update the stale OPEN status line + Seam-E prose to record WS5-D1 render, `urn:agentbox` BC20 ingest, ACSP 31400–31404, the 31402 dispatcher, and the `/wss/agent-events` consume side as built. Verified: status line already records all of this, with commit refs `e7771eb90`/`43a12e401`. | `agentbox/docs/reference/adr/ADR-026-cross-substrate-agent-loop-seams.md:4,52-59` |
| 3 | **ADR-020** | **DONE (2026-08-25).** Author the missing skill and fix the incoherent manifest (enabled against a non-existent skill). SKILL.md needs when-to-choose, the 7-step algorithm, negative-routing vs `sparc:coder`/`build-with-quality`/Edit, and 3 ICL exemplars. Dependency (ADR-018 kernel) already satisfied. Verified: `agentbox/skills/tree-search-coder/SKILL.md` exists (plus `references/`). | `agentbox/skills/tree-search-coder/SKILL.md` (new); confirm `agentbox.toml:621-628` |
...
| 5 | **ADR-060** | **DONE (2026-08-25).** Add the env-gated (default-OFF) drop-set filter: compute `private_opaque_ids` per caller, drop matching `(id,data)` before `encode_node_data_with_live_analytics_and_privacy`. ~80 lines per ADR-059 §Phase-4. Verified: `compute_private_opaque_ids`/`apply_drop_set` implemented and called in `position_updates.rs` (lines 286-290, 699-701). | `src/handlers/socket_flow_handler/position_updates.rs` |
...
| 7 | **ADR-070** | **DONE (2026-08-25).** Implement P1 D2.2 (constraint-force stability third criterion), D2.3 (input-edge NaN guard), and P2 sparse compute mask (largest open gap; drives Epic E.4 persona masking). One minor human confirm outstanding (D1.4 cap location — see tier c). Verified: D2.2/D2.3/D3.1 sparse-mask all present in `visionclaw_unified.cu` and `ontology_constraint_actor.rs`. | `crates/visionclaw-gpu` CUDA sources; `src/actors/gpu/` |
| 8 | **ADR-114** | **DONE (2026-08-25).** Build the RuVector seed leg (Oxigraph half already wired via `ontology_derived_handler.rs`): condensation job (ADR-113) writing per-class summaries via `memory_store` to namespace `ontology-classes` (bge-small 384-dim HNSW, never raw SQL); `ClassSummaryIndexRefreshed{changed_count}` refresh trigger on GitHubSync; liveness canary `memory_search` (ADR-119). Verified: `src/services/ontology_class_index.rs` (746 lines) implements the store, the refresh trigger wired into `github_sync_service.rs`, and the `liveness_canary`. | condensation job; GitHubSync hook; ADR-119 self-test |
