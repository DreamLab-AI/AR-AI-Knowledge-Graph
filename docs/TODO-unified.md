...
| C-14 | **Obsidian migration (branch `obsidian`, 2026-09-02)** — ADR-2040/2041/2042 (VisionClaw), ADR-2028/2029 (agentbox). Corpus: converted in place on `jjohare/logseq:obsidian` (archived source), then split history-preserving into private **`jjohare/visionGraph`** (`knowledge/`, `working/` vaults + `transcripts/` + publish pipeline). Shadow sync proved the conversion format-neutral (EXP-V08). Landed: vault readers/writers + Obsidian shortest-path link resolution, `knowledge` settings key, `vault-migrate` (title-echo/zero-byte fixes), `[vault]` path authority + Rune Notes tab, docs sweep. **Owner steps left:** (1) `.env`: `GITHUB_REPO=visionGraph`, `GITHUB_BASE_PATH=knowledge/pages,working/pages`; PAT must read the new private repo; (2) merge agentbox → VisionClaw (submodule bump) → relaunch dev stack; (3) `./agentbox.sh rebuild` for Rune + new entrypoint; (4) archive `jjohare/logseq` on GitHub. **Owner decisions recorded:** `_misc/` excluded from the narrativegoldmine publish walk; `/notes` (Logseq SPA) frozen at its last gh-pages deploy — rebuilding it with Obsidian tooling is a separate follow-up. **Pre-existing pipeline reds surfaced by the split (not fixed, corpus remediation):** `pipeline.iri_integrity` 102 new orphan refs beyond the stale baseline; `pipeline.build` exits 1 on `DUPLICATE_IRI` (`Open Source AI.md` / `Open-Source AI.md`) so `publish.yml` is likely already red upstream. Close-out report: `docs/gap-close-evidence/2026-09-02-obsidian-migration-closeout.md`. Agent embodiment (2026-09-02 evening): VisionClaw validates a NIP-98 header twice on `POST /api/bots/update` (`utils::auth::verify_access` then `settings::auth_extractor`) so the ADR-2002 single-use cache rejects the second check with `401 Token replayed` — use the session realm (`X-Nostr-Pubkey` + `X-Nostr-Token`) until fixed; no `agent_list` provider exists behind the bots relay (agent nodes appear only via `/api/bots/update`, dev driver at `scripts/dev/agent-beams-driver.cjs`, how-to `docs/how-to/operations/agent-beams-dev-driver.md`); agentbox emit/forwarder fixes (`d392bd4c2`) need the image rebuild. GPU physics 9423abdb3 (isolated-node peripheral shell anchored to 0.8×boundsSize) needs GPU re-verification after the next dev relaunch: isolated nodes settle at r≈320 and stay; the `Spatial grid exceeded cell-buffer` / `Low grid occupancy` warnings disappear; a hard positional clamp in `integrate_pass_kernel` and a connected-only AABB reduction remain as follow-ups. Also: `GitHubClient::get_full_path` still guesses with `path.starts_with(base)` (same family as the removed percent-decode; revisit under the vault path authority); `force_compute_actor::dag_rank_tests::directed_hierarchy_relation_accepts_only_class_subsumption` is self-contradictory on `main` (line 581 matches `hierarchical`, line 4585 asserts it must not) and needs an owner; diagram PNGs 01/04/06/07 still show Logseq labels (Nano Banana regen); `Two Heads Are Better Than One.md` has a corrupted `title:`; ADR-2040 tolerance removal due at review trigger. | visionclaw, agentbox, visionGraph | 2026-09-02 |
