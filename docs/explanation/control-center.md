...
that appears on demand, and the 168 fields are organised by **user intent** instead
...
- **Three-layer progressive disclosure.** L1 (five macro dials, always visible in
  the dock) covers the common case with one twist of a dial. L2 (eight semantic
  groups in a slide-out panel) covers deliberate tuning. L3 (168 individual field
  rows inside a group, or any one of them via `Ctrl+K`) covers the rare precise
  edit. Nobody has to wade through 168 rows to change bloom intensity.
...
| Export | What it is |
|---|---|
| `GROUP_DATA` / `REGISTRY` | The eight `RegistryGroup` objects, in rail order (hotkeys `1`–`8`) |
...
count (168), the exact per-group counts, the hotkey order, and — against
`registry/__tests__/legacy-paths.fixture.json` (captured from the old
`unifiedSettingsConfig.ts` before it was deleted) — that no path was added or
dropped in either direction. `pnpm gen:manifest`
(`client/scripts/emit-settings-manifest.ts`) regenerates the committed manifest
from the registry; CI fails if it has drifted.
...
Every field still lives at its original path; only the grouping changed. Totals
are enforced by the registry test above.
...
Two bespoke panels sit in the same rail below the eight groups, wrapping their
...
| `macros/MacroBar.tsx`, `useMacro.ts`, `registry/macros.ts` | The L1 dock row: five macro dials plus a physics toggle, a reset-layout action, and eight group-launcher icon buttons. |
| `panels/SettingsPanel.tsx` | The L2 slide-out: a left icon rail (eight groups + Solid/Ontology) and a body that renders a `GroupSection` or a bespoke panel, with a label/key/description/path/subgroup search filter. |
...
| `state/useControlCenterUI.ts` | Ephemeral UI state (open panel, active group, dock collapsed, echo-pulse flag, resizable panel width) — deliberately separate from the settings store so none of it can leak into the frozen 168-path contract. |
...
toggle, reset action, eight group-launcher icons, and an "Ask" button opening the
ported `CommandInput`) and the **StatusCluster** (top-right compact pill) are
visible.

| Key | Action |
|---|---|
| `1`–`8` | Open the slide-out panel to that semantic group (realises the old panel's dead `buttonKey` badges) |
...
| `Ctrl/Cmd+K` | **Not bound in the Control Center** — owned by the existing `CommandPalette`; `registry/paletteCommands.ts` registers one "reveal setting" command per field (168 of them) into that same registry, fuzzy-searchable by label or dot-path |
...
- **`CommandInput` (the Ask box) gains focus for the first time** →
  ...
  values across all 168 fields, one-time and intent-driven rather than at boot.
...
- **`AgentControlPanel` / `SkillsTab` were removed.** Neither was imported
  anywhere except each other and their own barrel re-export, with no mount
  point in the eight-group registry — they were deleted in dead-code pass 2
  along with 60+ other verified-orphan modules. The ~20-option
  `settings.agents.*` configuration surface they once exposed remains typed but
  unreachable through any UI; restoring it means building a registry group, not
  reviving the deleted components (they are recoverable from git history).
