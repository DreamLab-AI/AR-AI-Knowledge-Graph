...
**Source analysis:** `docs/analysis-visionclaw-x-understand-anything.md`
...
| H.3 | Block reference resolution: `((uuid))` in content becomes a `BlockRef` edge; aliased refs (`[alias]` over `((uuid))`) retain the alias as edge property | Test corpus with 100 block refs resolves all targets; broken refs flagged but don't crash |
