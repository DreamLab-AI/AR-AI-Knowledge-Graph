...
**Schema**: [Neo4j Schema — §2d Provenance Context](../reference/graph-schema.md#8-provenance-graph)
...
### 6.2 BeadStore Trait (`bead_store.rs`)
...
See [Neo4j Schema Reference — §2d Provenance Context](../reference/graph-schema.md#8-provenance-graph) for the full updated schema including `:Bead` lifecycle fields, `:BeadLearning` nodes, and `HAS_LEARNING` edges.
...
| Exhaustive outcome classification | `BeadOutcome` enum with no wildcard arms | `bead_types.rs` |
| `BeadStore` async trait | Same pattern, Neo4j backend instead of SQLite | `bead_store.rs` |
| Structured retrospectives | `BeadLearning` with what_worked/what_failed/reusable_pattern | [DDD — §2 Aggregates](../ddd/ddd-bead-provenance-context.md#2-aggregates) |
...
| [DDD Bead Context](../ddd/ddd-bead-provenance-context.md) | Bounded context analysis — aggregates, events, ports, ACL |
| [Neo4j Schema §2d](../reference/graph-schema.md#8-provenance-graph) | `:Bead`, `:BeadLearning`, `:NostrEvent` schema |
| [REST API — Debrief](../reference/rest-api.md#post-apibriefsbrief_iddebrief) | Endpoint that triggers bead creation |
